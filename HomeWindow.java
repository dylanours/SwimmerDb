...
public class HomeWindow extends JFrame {
	                ...
	                try { 
	                    ...
	                    if (result.get(0).equals("modmarker")){
	                        ...
	                        case "swimmer":
	                			...
	                			while (results.next()){
	                				namesFirst.add(results.getString("FirstName") + " " + results.getString("LastName"));
	                			}
	                			...
	                			String inName = null;
	                			while(inName == null){inName = (String)JOptionPane.showInputDialog(null,"Pick the name of the swimmer to modify:", "Input", JOptionPane.INFORMATION_MESSAGE, null, names, names[0]);}
	                			String[] firstLast=inName.split("\\s");
	                			String firstName = firstLast[0];
	                			String lastName = firstLast[1];
	                			query = "SELECT * FROM (SELECT SwimmerId, FirstName, MiddleName, BirthDate, Age, LastName, Gender, TeamId FROM Swimmer) AS SwimmerSub JOIN Records on SwimmerSub.SwimmerId = Records.SwimmerId NATURAL JOIN `Team` NATURAL JOIN `Event` WHERE ( FirstName = '" + firstName + "' AND LastName = '"+ lastName + "' )";
	                			results = stmt.executeQuery(query);
	                			results.next();
	                			int swimId = results.getInt("SwimmerId");
	                			String output = "SwimmerId: " + results.getInt("SwimmerId") + 
	                					"\nName: " + results.getString("FirstName") + " " + results.getString("MiddleName") + " " + results.getString("LastName") +
	                					"\nBirthDate: " + results.getString("BirthDate") + 
	                					"\n  Age: " + results.getInt("Age") +
	                					"\nGender: " + results.getString("Gender") + 
	                					"\nTeam Name: " + results.getString("TeamName") + 
	                					"\nEvent: " + results.getString("Distance") + " " + results.getString("Stroke");
	                			String[] attributes = {"FirstName", "MiddleName", "LastName", "BirthDate", "Age", "Gender", "Team", "Event"};
	                			textArea.setText(output);
	                			String attribute = null;
	                			attribute = (String)JOptionPane.showInputDialog(null,"Which attribute would you like to modify?", "Input", JOptionPane.INFORMATION_MESSAGE, null, attributes, "Pick an attribute...");                			
	                			if(attribute!=null){
	                				String val = JOptionPane.showInputDialog("Enter a new " + attribute + ".", "Type here...");
	                				String query2 = "UPDATE Swimmer SET " + attribute + "= \"" + val + "\" WHERE SwimmerId = \'" +swimId+"\'";
	                        ...
	                        case "meet":
	                			...
	                			String minName = null;
	                			while(minName == null){minName = (String)JOptionPane.showInputDialog(null,"Pick the name of the meet to modify:", "Input", JOptionPane.INFORMATION_MESSAGE, null, mnames, mnames[0]);}
	                			...
	                			mattribute = (String)JOptionPane.showInputDialog(null,"Which attribute would you like to modify?", "Input", JOptionPane.INFORMATION_MESSAGE, null, mattributes, "Pick an attribute...");                			
	                			if(mattribute!=null){
	                				String mval = JOptionPane.showInputDialog("Enter a new " + mattribute + ".", "Type here...");
	                				String mquery2 = "UPDATE Meet SET " + mattribute + "= \"" + mval + "\" WHERE SanctionNumber = \'" +mswimId+"\'";
	                        ...
	                        case "team":
	                			...
	                			String tinName = null;
	                			while(tinName == null){tinName = (String)JOptionPane.showInputDialog(null,"Pick the name of the team to modify:", "Input", JOptionPane.INFORMATION_MESSAGE, null, tnames, tnames[0]);}
	                			String tfirstName = tinName;
	                			tquery = "SELECT TeamId, HeadCoach, TeamName, PoolStreet, PoolNumber, PoolCity, PoolCountry FROM Team WHERE TeamName = '" + tfirstName +"'";
	                			tresults = stmt.executeQuery(tquery);
	                			tresults.next();
	                			int tswimId = tresults.getInt("TeamId");
	                			String toutput = "TeamId: " + tresults.getInt("TeamId") + 
	                					"\nTeam Name: " + tresults.getString("TeamName") +
	                					"\nHead Coach Name: " + tresults.getString("HeadCoach") +
	                					"\nPool Address: " + tresults.getString("PoolStreet") + " " + tresults.getInt("PoolNumber") + ", " + tresults.getString("PoolCity") + ", " + tresults.getString("PoolCountry");
	                			String[] tattributes = {"TeamName","HeadCoach", "PoolStreet", "PoolNumber","PoolCity","PoolCountry"};
	                			textArea.setText(toutput);
	                			String tattribute = null;
	                			tattribute = (String)JOptionPane.showInputDialog(null,"Which attribute would you like to modify?", "Input", JOptionPane.INFORMATION_MESSAGE, null, tattributes, "Pick an attribute...");                			
	                			if(tattribute!=null){
	                				String tval = JOptionPane.showInputDialog("Enter a new " + tattribute + ".", "Type here...");
	                				String tquery2 = "UPDATE Team SET " + tattribute + "= \"" + tval + "\" WHERE TeamId = \'" +tswimId+"\'";
	                    ...
	                    if(moveon){textArea.setText(Analysis(result, connect));}
	...
	public static ArrayList<String> Parse(String input){
		...
		String[] keywords = {"help","all","add", "compare", "modify", "record", "relay", "swimmer", "swimmers", "teams", "events", "meets", "records", "times","results", "team","meet","result", "delete", "fastest", "slowest", "male", "males", "boys", "men", "female", "females", "girls", "women", "freestyle", "free", "butterfly", "fly", "breaststroke", "breast", "backstroke", "back", "50", "100","200","400","800","1500",
				...
				if(count<(result.size()-1)){
					...
					case "events":
						...
						while(resultsEvents.next()){
							output = output + resultsEvents.getString("EventId") + ": " + resultsEvents.getString("Distance") + " " + resultsEvents.getString("Stroke") + "\n";
						}
						return output;
					case "teams":
	...
	public void Compare(Connection connect) throws SQLException {
		...
		setContentPane(contentPaneCompare);
            ...
            public void actionPerformed(ActionEvent e) {
        		...
        		try {
        			...
        			if(error){
            			...
            			query = "SELECT COUNT(records.Time) FROM (SELECT SwimmerId, FirstName, LastName FROM Swimmer) AS SwimmerSub JOIN Records on SwimmerSub.SwimmerId = Records.SwimmerId NATURAL JOIN `Event` WHERE ( FirstName = '" + firstName + "' AND LastName = '"+ lastName + "' )";
            			results = stmt.executeQuery(query);
            			int size=0;
            			while(results.next()){
            			...
            			query = "SELECT FirstName, LastName, Distance, Stroke, Records.Time FROM (SELECT SwimmerId, FirstName, LastName FROM Swimmer) AS SwimmerSub JOIN Records on SwimmerSub.SwimmerId = Records.SwimmerId NATURAL JOIN `Event` WHERE ( FirstName = '" + firstName + "' AND LastName = '"+ lastName + "' )";
            			results = stmt.executeQuery(query);
            			int cnt =0;
            			while(results.next()){
